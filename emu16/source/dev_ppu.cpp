...
void dev_ppu_tick(asm16_t *cpu, asm16_device_t * dev) {
    ...
    if (ppu->row_ < VIDEO_HEIGHT) {
        asm16_interrupt(cpu, INT_HSYNC);
        ppu->row_++;
    }
    else {
        asm16_interrupt(cpu, INT_VSYNC);
        ppu->row_ = 0;
        asm16_yield(cpu);
    }
}
...
bool dev_ppu_init(state_t * state) {
    ...
    asm16_device_t * dev = new asm16_device_t;
    ...
    asm16_add_device(state->cpu_, dev);
